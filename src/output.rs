...
pub fn print_text_results(results: &ScanResults) {
    ...
    for protocol in &results.protocol_support {
        let status = if protocol.supported {
            let color = if protocol.version.is_secure() {
                "YES".green()
    ...
    if let Some(fallback_scsv) = results.fallback_scsv_supported {
        ...
        if fallback_scsv {
            println!("  {}", "✓ Server protects against downgrade attacks".dimmed());
        } else {
            println!("  {}", "⚠ Server may be vulnerable to downgrade attacks".yellow().dimmed());
        }
        println!();
    }
    ...
    if let Some(secure_renego) = results.tls_renegotiation.secure_renegotiation {
        ...
        if !secure_renego {
            println!("    {}", "⚠ Server may be vulnerable to renegotiation attacks".yellow().dimmed());
        }
    } else {
    ...
    if let Some(client_renego) = results.tls_renegotiation.client_initiated_renegotiation {
        ...
        if client_renego {
            println!("    {}", "⚠ Server allows client renegotiation (DoS risk)".yellow().dimmed());
        } else {
    ...
    if let Some(compression) = results.tls_renegotiation.compression_supported {
        ...
        if compression {
            println!("    {}", "⚠ Server vulnerable to CRIME attack (CVE-2012-4929)".red().dimmed());
        } else {
    ...
    if let Some(heartbleed) = results.heartbleed_vulnerable {
        ...
        if heartbleed {
            println!("    {}", "⚠ CRITICAL: Server is vulnerable to Heartbleed attack!".red().bold());
            println!("    {}", "⚠ Private keys, passwords, and sensitive data may be leaked".red().dimmed());
            println!("    {}", "⚠ Immediate patching required!".red().dimmed());
        } else {
            println!("    {}", "✓ Server is protected against Heartbleed attacks".dimmed());
        }
        println!();
    }
    ...
    if !results.cipher_suites.is_empty() {
        ...
        let mut by_version: std::collections::HashMap<TlsVersion, Vec<_>> = std::collections::HashMap::new();
        
        for result in &results.cipher_suites {
            if result.supported {
                by_version.entry(result.cipher.protocol_version)
                    .or_insert_with(Vec::new)
                    .push(result);
            }
        }
        
        for version in &[TlsVersion::Tls13, TlsVersion::Tls12, TlsVersion::Tls11, TlsVersion::Tls10, TlsVersion::Ssl3, TlsVersion::Ssl2] {
            if let Some(ciphers) = by_version.get(version) {
                ...
                for cipher_result in ciphers {
                    ...
                    println!("  {} {}", prefix.yellow(), cipher_result.cipher.format_colored());
                }
                println!();
            }
        }
    ...
    if !results.key_exchange_groups.is_empty() {
        ...
        let classical_groups: Vec<_> = results.key_exchange_groups.iter()
            .filter(|g| !g.post_quantum && g.supported)
            .collect();
        let pq_groups: Vec<_> = results.key_exchange_groups.iter()
            .filter(|g| g.post_quantum && g.supported)
            .collect();
    ...
    if !results.certificate_chain.is_empty() {
        ...
        for (i, cert) in results.certificate_chain.iter().enumerate() {
            ...
            println!("  {}:", format!("{} Certificate (#{} in chain)", cert_type, i + 1).underline());
            println!();
            ...
            if let Some(curve_name) = &cert.ecc_curve_name {
                println!("      Algorithm:           {} ({})", cert.public_key_algorithm, curve_name);
            ...
            println!("      Not Before:          {}", cert.not_before.format("%Y-%m-%d %H:%M:%S UTC"));
            println!("      Not After:           {}", cert.not_after.format("%Y-%m-%d %H:%M:%S UTC"));
            ...
            println!("      SHA1 Fingerprint:    {}", cert.fingerprint_sha1.dimmed());
...
fn print_summary(results: &ScanResults) {
    ...
    let weak_ciphers: Vec<_> = results.cipher_suites.iter()
        .filter(|c| c.supported)
        .filter(|c| matches!(c.cipher.strength(), CipherStrength::Null | CipherStrength::Weak))
        .collect();
